...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 23 17:10:59 2022
...
file = '2A.GPM.DPR.V9-20211125.20220222-S002431-E015703.045368.V07A.HDF5'
...
ind1 = np.where((lon[:,0]>=49)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=54) #Where are lons <= -85
...
xlim = np.array([45.5,53]); ylim = np.array([-24,-15])
...
plt.title(r'GPM Overpass 02/22 0042 UTC $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 02/22 0042 UTC $log_{10 }(N_{W})$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,12)
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 02/22 0042 UTC KuPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,12)
cbar = plt.colorbar()
...
plt.title('02/22 0042 UTC Along-Track KuPR Percentiles', size = title_size)
...
norm_altitude = np.ones((y_bins.shape))*np.nan

for i in range(len(y_bins)):
    
    norm_altitude[i] = y_bins[i] - zero_deg_mean
    
    

#altitude_norm = 

plt.plot(ku_25,norm_altitude, label = '25p', color = 'k')
plt.plot(ku_50,norm_altitude,label = '50p', color = 'r')
plt.plot(ku_75,norm_altitude,label = '75p', color = 'b')
plt.plot(ku_95,norm_altitude,label = '95p', color = 'g')
