...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 20:46:58 2020
...
file = '2A.GPM.DPR.V8-20180723.20200412-S180410-E193644.034786.V06A.HDF5'
...
ind1 = np.where((lon[:,0]>=-88)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=-84) #Where are lons <= -85
...
plt.title(r'GPM Overpass 4/12 1804-1936 UTC $D_{0}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 4/12 1804-1936 UTC $N_{0}$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,15)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 4/12 1804-1936 UTC Ku-Band ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,15)
...
xlim = np.array([-92,-82]); ylim = np.array([29,37])
...
m.plot(lon[ind3[0],ray]+0.65,lat[ind3[0],27],'*w',markersize = 20, zorder=4,lw=0.25,alpha=0.75,markerfacecolor='k',markeredgewidth=0.25)
...
plt.title('GPM Overpass 4/12 1804-1936 UTC Ku-Band', size = 20)