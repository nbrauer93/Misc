...
file = '2A.GPM.DPR.V9-20211125.20230212-S144104-E161336.050901.V07A.HDF5'
...
ind1 = np.where((lon[:,0]>=73.5)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=76) #Where are lons <= -85
...
xlim = np.array([71,78]); ylim = np.array([-13,-21])
...
m.plot(lons[0:61],lats[0:61], 'k--', linewidth = 4)
...
plt.title('GPM Overpass 02/12 1459 UTC KuPR (Attenuation-Corrected)', size = 20, y = 1.04)
...
plt.title(r'GPM Overpass 02/12 1459 UTC $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 02/12 1459 UTC $log_{10 }(N_{W})$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,14)
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 02/12 1459 UTC KuPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,14)
cbar = plt.colorbar()
...
plt.title('Vertical Slope of KuPR in Liquid Phase 02/12 1459 UTC', size = 20, y = 1.04)
plt.show()
'''
