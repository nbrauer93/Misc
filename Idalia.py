...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  1 11:40:25 2023
...
file = '2A.GPM.DPR.V9-20211125.20230830-S021209-E034439.053990.V07B.HDF5'
...
xlim = np.array([-89,-82]); ylim = np.array([25,31])
...
m.plot(lons[0:94],lats[0:94], 'k--', linewidth = 4)
...
plt.title('08/30 0314 UTC KuPR (Attenuation-Corrected)', size = 20, y = 1.04)
...
plt.title(r'08/30 0314 UTC $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'08/30 0314 UTC $log_{10 }(N_{W})$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,16)
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'08/30 0314 UTC KuPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,16)
cbar = plt.colorbar()
...
xlim = np.array([-102,-96]); ylim = np.array([33,37])
m = Basemap(projection='cyl',lon_0=np.mean(xlim),lat_0=np.mean(ylim),llcrnrlat=ylim[0],urcrnrlat=ylim[1],llcrnrlon=xlim[0],urcrnrlon=xlim[1],resolution='i')
...
plt.title('Vertical Slope of KuPR in Liquid Phase 07/09 0929 UTC', size = 20, y = 1.04)
plt.show()
'''
