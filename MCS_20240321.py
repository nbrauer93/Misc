...
file = '2A.GPM.DPR.V9-20240130.20240321-S152200-E165514.057157.V07C.HDF5'
...
ind1 = np.where((lon[:,0]>=-94.5)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=-92) #Where are lons <= -85
...
m.plot(lons[0:116],lats[0:116], 'k--', linewidth = 4)
...
plt.title('03/21 1624 UTC KuPR (Attenuation-Corrected)', size = 20, y = 1.04)
...
plt.title(r'03/21 1624 UTC $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'03/21 1624 UTC $log_{10 }(N_{W})$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,14)
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'03/21 1624 UTC KuPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,14)
cbar = plt.colorbar()
...
plt.title('Vertical Slope of KuPR in Liquid Phase 03/21 1624 UTC', size = 20, y = 1.04)
plt.show()
'''