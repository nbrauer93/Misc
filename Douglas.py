...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 28 17:36:16 2020
...
file = '2A.GPM.DPR.V8-20180723.20200725-S151838-E165112.036402.V06A.HDF5'
...
ind1 = np.where((lon[:,0]>=-149)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=-146) #Where are lons <= -85
...
plt.title(r'GPM Overpass 7/25 1518-1631 UTC $D_{M}$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,15)
plt.colorbar().set_label(label = '[mm]', size = 18, extend = 'both')
plt.clim(0,3)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 7/25 1518-1631 UTC $N_{W}$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,15)
plt.colorbar().set_label(label = r'[#$cm^{-3}$]', size = 18, extend = 'both')
plt.clim(0,80)
plt.text(x_loc, y_loc, label)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 7/25 1518-1631 UTC KuPr ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,15)
#plt.colorbar(pm, label = 'dBZ')
plt.colorbar().set_label(label='[dBZ]',size=18, extend = 'both')
plt.clim(12,60)
...
xlim = np.array([-157,-146]); ylim = np.array([17,22])
...
plt.title('GPM Overpass 7/25 1518-1631 UTC KuPr', size = 20)
