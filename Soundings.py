...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 16 16:21:30 2020
...
temp_array_raw = df['temperature']
temp_array = pd.Series.to_numpy(temp_array_raw)

freezing_index = np.where(temp_array<0.5)[0]

print(temp_array[freezing_index][0], 'degC')


pressure_array_raw = df['pressure']
pressure_array = pd.Series.to_numpy(pressure_array_raw)
pressure_freezing = pressure_array[freezing_index]




print(pressure_freezing[0], 'mb')


#What about for height?

height_array_raw = df['height']
height_array = pd.Series.to_numpy(height_array_raw)
freezing_level = height_array[freezing_index]

print(freezing_level[0], 'meters') 
...
plt.title('DNR 20220317 00Z', size = 44)
