...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 28 15:21:37 2020
...
file = '2A.GPM.DPR.V8-20180723.20200725-S121327-E134601.036400.V06A.HDF5'
...
ind1 = np.where((lon[:,0]>=-98)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=-96) #Where are lons <= -85
...
plt.title(r'GPM Overpass 7/25 1213-1346 UTC $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 7/25 1213-1346 UTC $N_{W}$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,15)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 7/25 1213-1346 UTC KuPr ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,15)
...
xlim = np.array([-100,-93]); ylim = np.array([26,31])
...
plt.title('GPM Overpass 7/25 1213-1346 UTC KuPr', size = 20)