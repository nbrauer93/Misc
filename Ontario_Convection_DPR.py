...
file = '2A.GPM.DPR.V9-20211125.20220621-S004035-E021306.047220.V07A.HDF5'
...
ind1 = np.where((lon[:,0]>=-83)) #Where are the DPR lons >= -100
ind2 = np.where((lon[:,0])<=-80) #Where are lons <= -85
...
xlim = np.array([-84,-76]); ylim = np.array([46,52])
...
plt.title('GPM Overpass 06/21 0118 UTC KuPR (Attenuation-Corrected)', size = 20, y = 1.04)    
...
plt.title(r'GPM Overpass 06/21 0118 UTC $D_{M}$ ', size = 20)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 06/21 0118 UTC $log_{10 }(N_{W})$ ', size = 20)
plt.xlim(dist[0],dist[1])
plt.ylim(0,12)
cbar = plt.colorbar()
cbar.ax.tick_params(labelsize = label_size)
...
plt.xlabel('Along Track Distance (km)', size = 20)
plt.ylabel('Altitude (km)', size = 20)
plt.title(r'GPM Overpass 06/21 0118 UTC KuPR ', size = 20)
#plt.xlim(300,450)
plt.xlim(dist[0], dist[1])
plt.ylim(0,12)
cbar = plt.colorbar()
...
plt.title('Vertical Slope of KuPR in Liquid Phase 06/21 0118 UTC', size = 22, y = 1.04)
