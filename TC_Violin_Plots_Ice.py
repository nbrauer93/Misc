...
files_dl  = glob.glob('0309_slope_ice_dl_eyewall*')
files_dr  = glob.glob('0309_slope_ice_dr_eyewall*')
files_ul  = glob.glob('0309_slope_ice_ul_eyewall*')
files_ur  = glob.glob('0309_slope_ice_ur_eyewall*')
...
plt.title(r'$\bf{i)}$ Slope of KuPR in Ice Phase 03/09 1009 UTC (Svr TS)', size = title_font_size, y = 1.02)
ax.set_xticklabels(['DL','DR','UL','UR'],size = label_font_size)
ax.set_yticklabels(ax.get_yticks(), size = label_font_size)
plt.axhline(y = 0, xmin = -1.5, xmax = 4, color = 'k', linewidth =  3.0, linestyle = '--')
plt.savefig('0309_ice.png', dpi = 300)
plt.show()
