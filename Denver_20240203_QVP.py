...
radar_file = 'KFTG20240203_233901_V06'
...
times = []
qvps_z = np.ones((135,1832))*np.nan
qvps_zdr = np.ones((135,1832))*np.nan
qvps_rhohv = np.ones((135,1832))*np.nan
...
qvps_z[qvps_rhohv<0.7] = np.nan
qvps_zdr[qvps_rhohv<0.7] = np.nan
...
plt.title(r'KFTG 02/03 $Z_{H}$', size = title_size)
x = [0,27,54,81,108,135]
labels = np.array(['1200','1400','1600','1800','2000','2200','2400'])
plt.xticks(x,labels,size = font_size)
plt.yticks(size = font_size)
plt.savefig('Z_qvp.png', dpi = 300)
plt.show()
...
plt.title(r'KFTG 02/03 $Z_{DR}$', size = title_size)

plt.xticks(x,labels,size = font_size)
plt.yticks(size = font_size)
plt.savefig('zdr_qvp.png', dpi = 300)
plt.show()
...
plt.title(r'KFTG 02/03 $\rho_{HV}$', size = title_size)

plt.xticks(x,labels,size = font_size)
plt.yticks(size = font_size)
plt.savefig('rhohv_qvp.png', dpi = 300)
plt.show()
